...
def page_three(c):
    ...
    c.setFillColor(PALE)
    c.roundRect(M, y - 88, CW, 88, 10, fill=1, stroke=0)
    c.setFillColor(INK)
    c.setFont("MalgunBold", 9.5)
    c.drawString(M + 16, y - 21, "충남도립대학교 · 호텔조리학과")
    c.setFont("MalgunBold", 9)
    c.drawString(M + 16, y - 49, "리눅스마스터 2급")
    c.setFillColor(MUTED)
    c.setFont("Malgun", 8)
    c.drawString(M + 110, y - 49, "2026.08.01 시험 예정")
    c.setFillColor(INK)
    c.setFont("MalgunBold", 9)
    c.drawString(M + 16, y - 69, "정보처리기능사")
    c.setFillColor(MUTED)
    c.setFont("Malgun", 8)
    c.drawString(M + 110, y - 69, "필기 합격 경험 · 재응시 준비")
    y -= 112
