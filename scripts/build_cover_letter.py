...
def build():
    ...
    hero = Table([[Paragraph("저를 가장 잘 설명하는 단어는 ‘생존력’입니다.", HERO)]], colWidths=[W - 2 * M])
    ...
    add_section(story, "01", "성장 과정 · 변화 속에서 제 역할을 만드는 생존력", [
        "저에게 생존력은 주어진 상황을 버티는 능력이 아닙니다. 환경이 달라졌을 때 변화를 빠르게 받아들이고, 필요한 것을 배우며, 결국 제 역할을 만들어내는 능력입니다. 프로게이머, 요식업, 개발은 서로 다른 분야였지만 저는 매번 초보자의 자리에서 시작했습니다. 반복적인 연습과 피드백으로 실력을 쌓아 프로게이머 팀의 리더가 되었고, 군 복무 이후에는 요식업의 기본 업무부터 다시 익혀 주방 운영을 책임지는 주방장이 되었습니다.",
        "리더였다는 결과보다 그 자리까지 올라가는 과정을 여러 번 경험했다는 점이 저의 강점입니다. 지시와 피드백을 받아들이고 기초부터 반복했으며, 제가 하고자 했던 분야에서는 제 몫을 해낼 때까지 쉽게 포기하지 않았습니다. 개발 역시 비전공 신입으로 시작했지만 웹 서비스 구현에서 Azure와 Kubernetes 배포·운영까지 학습 범위를 넓혔습니다.",
    ])
    add_section(story, "02", "성격의 장단점 · 사람들과 어울리며 끝까지 해결합니다", [
        "저는 새로운 조직에 적응하고 사람들과 신뢰 관계를 형성하는 데 자신이 있습니다. 프로게이머 팀과 요식업 현장처럼 다양한 역할의 사람들이 함께 움직이는 환경에서 상대를 존중하는 태도와 기본적인 예의를 중요하게 생각해왔습니다. 제 의견을 먼저 내세우기보다 조직의 방식과 맡은 역할을 이해하고, 지시받은 업무를 정확하게 수행합니다. 조건이 달라지면 상황과 사람에 맞춰 우선순위를 조정하며 유연하게 대응합니다.",
        ...
        "또한 빠르게 실행하는 데 비해 기록과 계획이 뒤늦게 따라오는 점을 보완하기 위해 Git의 브랜치와 커밋으로 작업 단위를 나누고, AI 대화도 프로젝트와 주제별로 분리해 관리하고 있습니다. 현재는 작업 우선순위와 진행 상태를 관리하는 AntiADHD 프로젝트를 개발하며 저에게 맞는 업무 관리 체계를 직접 만들고 있습니다.",
    ])
    story.append(PageBreak())

    page_header(story, "서비스를 사용자에게 전달하는 사람", "지원 동기와 클라우드 엔지니어로서의 성장 목표")
    add_section(story, "03", "지원 동기 및 직무 역량 · 클라우드에서 찾은 답", [
        "주방 일은 제가 이미 잘하고 있었고 필요하다면 언제든 다시 해낼 수 있는 일이었습니다. 하지만 빠르게 변화하는 AI 시대에 익숙한 능력만 고집해서는 경쟁력을 유지하기 어렵다고 판단했습니다. 저는 단순히 새로운 직업을 찾기보다 어릴 때부터 관심을 두고 꿈꿔왔던 개발에 도전했습니다. Java와 Spring 기반 백엔드, React 기반 프론트엔드, 데이터베이스와 AI 기술을 배우며 인증, 상품, 외부 API, 이미지 업로드와 AI 분석 결과 화면 등을 구현했습니다.",
        "AI가 개발 생산성을 높이는 모습을 보며 기능 구현을 넘어 변화 속에서도 오래 필요한 역할이 무엇인지 고민했습니다. AI가 개발을 돕더라도 서비스를 실행하고 연결하며, 보호하고 장애에 대응해 사용자에게 안정적으로 전달하는 책임은 사라지지 않는다고 생각했습니다. 기능이 구현됐다는 사실만으로 서비스가 완성되는 것은 아닙니다. 사용자가 안정적으로 이용할 수 있어야 가치가 전달되며, 저는 그 기반을 책임지는 클라우드 인프라에서 답을 찾았습니다.",
    ...
    add_section(story, "04", "입사 후 포부 · 기본 업무부터 운영 책임까지", [
        ...
        "저에게 생존은 변화를 외면하지 않고 필요하다면 다시 배우는 적극적인 선택입니다. 새로운 조직에서도 기본부터 배우며 제 역할을 만들고, 서비스의 배포부터 운영과 장애 대응까지 책임지는 클라우드 엔지니어가 되겠습니다.",
    ])
