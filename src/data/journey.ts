...
export const journeyGroups: JourneyGroup[] = [
    ...
    items: [
      {
        ...
        summary: '모바일 생산성 앱과 온프레미스 k3s 운영 환경을 함께 구축',
        growthLabel: 'On-premise Kubernetes',
        projectType: '개인 프로젝트',
        status: 'In Progress',
        role: '모바일·백엔드 개발 및 온프레미스 Kubernetes 구축·운영',
        technologies: [
          'React Native · Expo',
          ...
          'Prometheus · Grafana',
          'GitHub Actions · GHCR',
          'OpenAI · Offline-first',
        ],
        work: [
          '회원 인증, 일정·루틴·목표, Focus Mode와 Daily Review를 React Native 앱과 Spring Boot API로 구현',
          'AI 시작 계획과 음성 일정을 비동기 작업으로 분리하고 JWT·Refresh Token 기반 인증을 적용',
          '단일 노드 k3s에 Traefik Ingress, PostgreSQL, ConfigMap·Secret, liveness/readiness probe를 구성',
          'GitHub Actions와 GHCR 이미지 배포, Prometheus·Grafana 관측, PostgreSQL 백업·복구 흐름을 구축',
          '서버 연결이 끊겨도 일정을 사용할 수 있도록 오프라인 저장과 재연결 동기화를 구현',
        ],
        limitation:
          '지속적으로 접근해야 하는 생활 서비스에 개인 홈 서버를 선택하면서 상시 전원, 외부 접근, 장비·전력 의존성과 단일 노드 가용성을 직접 책임해야 했습니다.',
        problemSolving: [
          'AI 요청을 별도 Worker Deployment로 분리해 일반 CRUD 요청과 장애 영역을 나눴습니다.',
          '홈 서버 중단과 외부 연결 한계를 보완하기 위해 일정 도메인부터 오프라인 저장·재연결 동기화를 적용했습니다.',
          '운영 DB 마이그레이션 실패 시 직전 GHCR 이미지로 롤백하고 실제 스키마 기반 dry-run 후 재배포했습니다.',
        ],
        learned:
          'Kubernetes를 직접 구축·운영하는 경험은 얻었지만, 사용하고 싶은 기술을 먼저 선택하면 운영 구조가 서비스 특성과 맞지 않을 수 있다는 점을 배웠습니다.',
        nextJourney: {
          title: 'MindOff',
          description:
            '기술을 먼저 정하지 않고 서비스의 접근성·가용성·운영·비용 요구사항을 분석한 뒤 적합한 AWS 서비스를 선택합니다.',
        },
  ...
  },
  {
    ...
    items: [
      {
        id: 'mindoff',
        image: '/images/projects/mindoff.svg',
        name: 'MindOff',
        phase: 'AWS Architecture',
        period: '2026.08 → Present',
        summary: '“내가 기억하지 않아도 된다”를 목표로 한 모바일 생활비서와 AWS 아키텍처 설계',
        growthLabel: 'Requirement-driven Architecture',
        projectType: '개인 프로젝트',
        status: 'In Progress',
        role: '제품 기획·프로토타입 개발 및 AWS 아키텍처 설계',
        technologies: [
          'React Native',
          'Spring Boot',
          'PostgreSQL',
          'AWS Architecture',
          'Managed Service · Event Driven',
        ],
        work: [
          '영수증, 냉장고 유통기한, 생활용품, Need List와 구독 관리 흐름을 제품 프로토타입으로 구현',
          '최소 입력으로 반복 생활 정보를 기록하고 필요한 시점에 알려주는 모바일 생활비서를 기획·개발 중',
          '상시 API는 ECS Fargate·RDS PostgreSQL, 인증은 Cognito를 사용하는 Target Architecture를 설계 중',
          '영수증 이미지는 S3·Lambda·Textract, 시간 기반 알림은 EventBridge Scheduler·SNS로 역할을 분리해 설계 중',
          'ECR, CloudWatch, IAM, Secrets Manager를 배포·관측·권한·비밀 관리 계층으로 검토 중',
        ],
        limitation:
          '제품 프로토타입은 공개되어 있지만 AWS 인프라는 설계·개발 단계입니다. 개별 AWS 서비스는 실제 운영 경험이 아닌 Target Architecture로 구분합니다.',
        learned:
          'AntiADHD의 온프레미스 운영 회고를 바탕으로, 기술을 먼저 선택하지 않고 접근성·가용성·워크로드·비용 요구사항에서 인프라를 결정하는 과정을 검증하고 있습니다.',
        nextJourney: {
          title: 'Infrastructure as Code',
          description:
            '검증한 AWS 아키텍처를 Terraform으로 반복 가능하게 재현할 수 있도록 코드로 관리합니다.',
        },
        liveUrl: 'https://mindoff-project-preview.vercel.app/',
        icon: CloudCog,
      },
    ],
  },
...
export const futureRoadmap: RoadmapItem[] = [
  {
    label: 'Current',
    title: 'MindOff · AWS Architecture',
    description:
      '제품 프로토타입을 개발하며 서비스 요구사항에 따라 Managed Service와 Event Driven Architecture를 설계하는 단계입니다.',
  },
  {
    label: 'Next',
    title: 'Terraform · Infrastructure as Code',
    description:
      '선택한 AWS 리소스를 코드 기반으로 재현하고 변경 이력을 관리합니다.',
  },
  {
    label: 'Next',
    title: 'CI/CD · Cloud Observability',
    description:
      'ECR 기반 배포 자동화와 CloudWatch 로그·메트릭·알림 체계를 연결합니다.',
  },
  {
    label: 'Growth',
    title: 'Multi Cloud Perspective',
    description:
      'Azure·온프레미스·AWS 경험을 비교하며 서비스에 적합한 운영 환경을 판단하는 관점을 넓힙니다.',
  },
  {
    label: 'Future',
